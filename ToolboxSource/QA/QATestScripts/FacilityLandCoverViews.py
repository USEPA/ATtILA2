...
def runATtILA(paramDict, iteration):  
  ...
  metric.runFacilityLandCoverViews(
                              ...
                              paramDict["Facility_feature"],
                              paramDict["View_radius"], 
                              paramDict["View_threshold"],
                              outTable,
...
paramCombosList = list(itertools.product(*FLCV_options.testInputs.values()))
...
for iteration, params in enumerate(paramCombosList): 
  paramDict = dict(zip(FLCV_options.testInputs.keys(), params))
